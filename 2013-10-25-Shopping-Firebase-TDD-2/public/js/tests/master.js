...
test('Add Product', function(){
  ...
  equal(db.products[0].salePrice(), 450, 'product should have a sale price');
...
test(' Add order test', function(){
  ...
  equal($('#cart tbody tr').length, 0, 'should have no rows in tbody after purchase');
  equal($('#cart-grand').text(), '', 'should be no grand total after purchase');
  equal($('#select-customer').val(), 'Select Customer', 'drop down should default after purchase');

  equal(db.orders.length, 1, 'should be 1 order after purchase');
  ok(db.orders[0] instanceof Order, 'order should be an instance of Order');
  ok(db.orders[0].id, 'order should have an id after purchase');
...
test('Remove Cart Items', function(){
  expect(6);
  ...
  $('#cart tbody tr:nth-child(2)').trigger('click');

  equal($('#cart tbody tr').length, 2, 'should be 2 items in body');
  equal($('#cart tbody tr:nth-child(2) .product-name').text(), 'Apple TV', 'name should be Apple TV');
  equal($('#cart tfoot tr #cart-count').text(), '3', 'should have 3 items in cart');
});
